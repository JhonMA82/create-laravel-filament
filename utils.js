...
export async function ensurePhpunitAppLocaleEn(projectPath) {
  ...
  try {
    const xmlData = await fs.readFile(phpunitPath, 'utf-8')
    const parser = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '@_' })
    let jsonObj = parser.parse(xmlData)

    if (!jsonObj.phpunit.php) {
      jsonObj.phpunit.php = { env: [] }
    }

    let appLocaleEnv = jsonObj.phpunit.php.env.find(e => e['@_name'] === 'APP_LOCALE')
    if (appLocaleEnv) {
      appLocaleEnv['@_value'] = 'en'
    } else {
      jsonObj.phpunit.php.env.push({ '@_name': 'APP_LOCALE', '@_value': 'en' })
    }

    // Fix the colors attribute to have a proper value
    if (jsonObj.phpunit['@_colors'] === '') {
      jsonObj.phpunit['@_colors'] = 'true'
    }

    const builder = new XMLBuilder({ ignoreAttributes: false, attributeNamePrefix: '@_', format: true })
    const xmlContent = builder.build(jsonObj)
    await fs.writeFile(phpunitPath, xmlContent)
